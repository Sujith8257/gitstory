...
export const fetchUserStory = async (
  ...
  try {
    ...
    const [
      ...
      fetch(
        `${GITHUB_API_BASE}/search/issues?q=author:${username}+type:pr+created:2025-01-01..2025-12-31&per_page=1`,
        { headers }
      ),
      // Issues authored in 2025
      fetch(
        `${GITHUB_API_BASE}/search/issues?q=author:${username}+type:issue+created:2025-01-01..2025-12-31&per_page=1`,
        { headers }
      ),
      // PR reviews in 2025
      fetch(
        `${GITHUB_API_BASE}/search/issues?q=reviewed-by:${username}+-author:${username}+type:pr+created:2025-01-01..2025-12-31&per_page=1`,
        { headers }
      ),
    ]);
    ...
    const topRepos: Repository[] = topCandidates.slice(0, 5).map((c) => ({
      name: c.repo.name,
      description:
        c.repo.description || "A project that speaks through its code.",
